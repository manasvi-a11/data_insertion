...
def insert_documents_from_csv(file_path, db, collection_name):
    """Reads profession data from a CSV file and inserts it into MongoDB."""
    profession_df = read_csv_safe(file_path)
    
    if profession_df is None or profession_df.empty:
        print("No data found in the CSV file. Aborting insert.")
        return

    collection = db[collection_name]
    
    for row in profession_df.itertuples(index=False):
        document = {
            'title': row[0], 
            'iconPath': row[1],    
            'rank': row[2],        
        ...
        try:
            ...
            print(f"Inserted document for profession '{row[0]}'")
        except Exception as e:
            print(f"Error inserting profession '{row[0]}': {e}")
...
def main():
    ...
    try:
        ...
        csv_file_path = "NTConfiguration.profession - NTConfiguration.profession.csv.csv"  
