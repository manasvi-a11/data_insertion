...
def insert_documents_from_csv(file_path, db, collection_name):
    """Reads qualification data from a CSV file and inserts it into MongoDB."""
    qualification_df = read_csv_safe(file_path)
    ...
    if 'Qualification' not in qualification_df.columns:
        print("The CSV file does not contain a 'department' column. Aborting insert.")
        return

    for item in qualification_df['Qualification'].dropna():
        document = {
            'title': item,
            'status': 'verified',
            'createdBy': 'admin',
        ...
        try:
            ...
            print(f"Inserted document for qualification '{item}'")
        except Exception as e:
            print(f"Error inserting qualification '{item}': {e}")
...
def main():
    ...
    try:
        ...
        collection_name = 'Qualification'
        csv_file_path = "QUALIFICATION - Sheet1.csv" 
