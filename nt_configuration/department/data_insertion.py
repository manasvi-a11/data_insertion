...
def insert_documents_from_csv(file_path, db, collection_name):
    """Reads department data from a CSV file and inserts it into MongoDB."""
    department_df = read_csv_safe(file_path)
    ...
    if 'Departments' not in department_df.columns:
        print("The CSV file does not contain a 'department' column. Aborting insert.")
        return

    for item in department_df['Departments'].dropna():
        document = {
            'title': item,
            'status': 'verified',
            'createdBy': 'admin',
        ...
        try:
            ...
            print(f"Inserted document for department '{item}'")
        except Exception as e:
            print(f"Error inserting department '{item}': {e}")
...
def main():
    ...
    try:
        ...
        csv_file_path = "Departments for Endeca search - Sheet1.csv" 
