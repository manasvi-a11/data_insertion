...
def insert_documents_from_csv(file_path, db, collection_name):
    """Reads consulatation data from a CSV file and inserts it into MongoDB."""
    consultation_df = read_csv_safe(file_path)
    
    if consultation_df is None or consultation_df.empty:
        print("No data found in the CSV file. Aborting insert.")
        return

    collection = db[collection_name]
    
    # Assuming that the CSV has a 'consultation' column for inserting data
    if 'Consultation' not in consultation_df.columns:
        print("The CSV file does not contain a 'consultation' column. Aborting insert.")
        return

    for item in consultation_df['Consultation'].dropna():
        document = {
            'title': item,
            'status': 'verified',
            'createdBy': 'admin',
        ...
        try:
            ...
            print(f"Inserted document for consultation '{item}'")
        except Exception as e:
            print(f"Error inserting consultation '{item}': {e}")
...
def main():
    ...
    try:
        ...
        collection_name = 'Consultation'
        csv_file_path = "Procedures & Consultation - Consultation.csv" 
