...
def insert_documents_from_csv(file_path, db, collection_name):
    """Reads area data from a CSV file and inserts it into MongoDB."""
    area_df = read_csv_safe(file_path)
    ...
    # Assuming that the CSV has a 'area' column for inserting data
    if 'Area' not in area_df.columns:
        print("The CSV file does not contain a 'area' column. Aborting insert.")
        return

    for item in area_df['Area'].dropna():
        document = {
            'title': item,
            'status': 'verified',
            'createdBy': 'admin',
        ...
        try:
            ...
            print(f"Inserted document for area '{item}'")
        except Exception as e:
            print(f"Error inserting area '{item}': {e}")
...
def main():
    ...
    try:
        ...
        csv_file_path = "Area - NTConfiguration.csv"  
